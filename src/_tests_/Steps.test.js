import { sortAndFilter } from '../components/steps/helpers.js';
...
describe("Step filter logic", () => {
  let sorted = sortAndFilter(input)
  test("it should have four steps sorted by stepNumber", () => {
    expect(sorted[0].stepNumber).toEqual('1')
    expect(sorted[1].stepNumber).toEqual('2')
    expect(sorted[2].stepNumber).toEqual('3')
    expect(sorted[3].stepNumber).toEqual('4')
  })
  test ("it should have one object under versionContent", () => {
    expect(typeof(sorted[2].versionContent)).toEqual("object");
  })
  test("it should choose the most recent effectiveDate", () => {
    expect(sorted[2].versionContent.effectiveDate).toEqual("2019-04-04T05:04:05.000Z")
  })
})
